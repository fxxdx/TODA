...
class tf_encoder(nn.Module):
    def __init__(self, configs):
        ...
        self.pool = nn.AdaptiveAvgPool1d(output_size=configs.fourier_modes*2)

    def forward(self, x):
        ef, out_ft = self.freq_feature(x)
        # print(ef.shape)
        # print(self.avg(ef).shape)
        # print("squeeze:", self.pool(self.avg(ef)).shape)
        # print("111: ", torch.squeeze(self.pool(self.avg(ef)),dim=1).shape)
        ef = F.relu(self.bn_freq(torch.squeeze(self.pool(self.avg(ef)),dim=1)))
...
class tf_decoder(nn.Module):
    def __init__(self, configs):
        ...
        self.convT = torch.nn.ConvTranspose1d(configs.dim1, self.sequence_len, self.input_channels, stride=1)
        # self.convT = torch.nn.ConvTranspose1d(configs.final_out_channels, self.input_channels, kernel_size=128, stride=1, output_padding=0)

        self.dim2 = configs.dim2
        self.modes = configs.fourier_modes

    def forward(self, f, out_ft):
        # print("f: ",f.shape)
        # print("out_ft:", out_ft.shape)
        # print("ffit: ", torch.fft.irfft(out_ft, n=3000).shape)
        x_low = self.bn1(torch.fft.irfft(out_ft, n=self.dim2))   # reconstruct  time series by using low frequency frequency features
        # print("low: ", x_low.shape)
        et = f[:,self.modes*2:]
        # print("et: ",et.shape)

        # print("bn2:", self.convT(et.unsqueeze(2)).shape)
...
class RAINCOAT(Algorithm):
    ...
    def update(self, src_x, src_y, trg_x):
        ...
        # print("src:", src_recon.shape)
        # print("srcx:", src_x.shape)
