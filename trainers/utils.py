...
def scaling(x, sigma=1.1):
    ...
    return torch.tensor(np.concatenate((ai), axis=1))


def permutation(x, max_segments=5, seg_mode="random"):
    # print("xdata.shape: ", x.shape)
    orig_steps = np.arange(x.shape[2])

    num_segs = np.random.randint(1, max_segments, size=(x.shape[0]))

    ret = np.zeros_like(x.cpu())
    for i, pat in enumerate(x.cpu()):
        # print("i: ", i)
        # print("pat: ", pat.shape)
        if num_segs[i] > 1:
            if seg_mode == "random":
...
def trans(timesteps, features):
    # print(features.shape)
    seq_len = features.shape[2]
    z_aug1 = features.transpose(1, 2)

    t_samples = torch.randint(seq_len - timesteps, size=(1,)).long()  # randomly pick time stamps
